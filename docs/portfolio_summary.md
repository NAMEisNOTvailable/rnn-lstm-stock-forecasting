...
This repository presents a financial time-series forecasting coursework project as a reviewer-friendly machine-learning portfolio artifact. The core experiment compares SimpleRNN and LSTM models on Google and Occidental Petroleum stock-price series using 60-timestep windows.
...
- The reusable `src/` package extracts the parts reviewers can validate quickly: data cleaning, train/test scaling, sequence construction, test-window handling, forecast metrics, and a persistence baseline.
...
The persistence baseline in `results/baseline_metrics.csv` gives reviewers a simple comparator. A neural model should be discussed against this baseline rather than judged only by visual fit.
